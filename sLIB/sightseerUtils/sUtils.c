...
float sut_pos_pid_update(sut_pos_pid_t* pid, float measured_value, float dt_s) {
    ...
    // 积分限幅
    sut_fconstrain(&pid->integral, pid->integral_min, pid->integral_max);
    // 计算微分
    ...
    // 限制输出范围
    sut_fconstrain(&output, pid->output_min, pid->output_max);
    return output;
}
...
float sut_inc_pid_update(sut_inc_pid_t* pid, float measured_value, float dt_s) {
    ...
    // 限制输出范围
    sut_fconstrain(&pid->output, pid->output_min, pid->output_max);
    // 保存当前误差为下一次计算的"上一次误差"
    pid->prev_error = error;
    return pid->output;
}
